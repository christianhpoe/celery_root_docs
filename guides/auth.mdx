...
```bash
export CELERY_CNC_BASIC_AUTH="admin:secret,ops:another"
```
...
```bash
export CELERY_CNC_AUTH_PROVIDER="github"
export CELERY_CNC_OAUTH2_KEY="<client_id>"
export CELERY_CNC_OAUTH2_SECRET="<client_secret>"
export CELERY_CNC_OAUTH2_REDIRECT_URI="http://127.0.0.1:8000/login"
```
...
```bash
export CELERY_CNC_AUTH="allowed-emails:.*@example.com"
```

## Custom auth handler

Set `CELERY_CNC_AUTH_PROVIDER` to a callable path to run custom logic:

```bash
export CELERY_CNC_AUTH_PROVIDER="your_module.auth:allow_request"
```
